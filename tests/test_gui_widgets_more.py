...
class TestSummaryPanelMore:
    def test_refresh_for_natural_and_artificial_rows(self, qapp):
        ...
        panel.set_generated_waves([art])

        text = panel._overview.text()
        assert "天然波 1 条" in text
        assert "人工波 1 条" in text
        assert panel._table.rowCount() == 2
        assert panel._table.item(0, 0).text() == "天然波"
        assert panel._table.item(1, 0).text() == "人工波"
        assert panel._table.item(0, 1).text() == "RSN5"
        assert panel._table.item(1, 1).text() == "art1"

    def test_set_dark_only_updates_flag(self, qapp):
        from seiswave.gui.panels.summary_panel import SummaryPanel

        panel = SummaryPanel(dark=False)
        panel.set_dark(True)
