...
# (所有基础函数与 test_lm_prototype.py 相同，省略重复部分)
# 这里只保留修改的测试部分
...
def compute_spectrum(acc, dt, periods, zeta=0.05):
    return CachedSpectrumComputer(acc, dt, zeta).compute(periods)
...
def wfunc_new(n, dt, itm, T, zeta, taper_cycles=3.0):
    ...
    if abs(zeta) < 0.999 and abs(1.0 - zeta**2) > 1e-30:
        ...
        alpha = zeta * omega
    # 关键：deltaT 偏移使小波峰值与 SDOF 响应峰值对齐
...
def run_experiment(acc, n, dt, zeta, P_ctrl, nP_ctrl, SPAT_ctrl, periods, target_sa, PGA,
                   stages, max_iter, tol, label):
    """运行单个实验配置"""
    t0 = time.time()
    acc_f, stage_results = progressive_adjustspectra(
        acc.copy(), PGA, n, dt, zeta, P_ctrl, nP_ctrl, SPAT_ctrl,
        tol=tol, max_iter=max_iter, use_new_wavelet=True, stages=stages
    )
    t_elapsed = time.time() - t0
    pk = float(np.max(np.abs(acc_f)))
    sa = compute_spectrum(acc_f, dt, periods, zeta)
    e = (sa - target_sa) / np.maximum(target_sa, 1e-30)
    aerror = float(np.sqrt(np.mean(e * e)))
    merror = float(np.max(np.abs(e)))
    return {
        'label': label,
...
if __name__ == "__main__":
    ...
    experiments = [
        # 基础配置
        {'stages': [0.5, 0.75, 1.0], 'max_iter': 20, 'tol': 0.05, 'label': '基础3阶段(20iter)'},
        {'stages': [0.5, 0.75, 1.0], 'max_iter': 30, 'tol': 0.05, 'label': '基础3阶段(30iter)'},
        {'stages': [0.5, 0.75, 1.0], 'max_iter': 40, 'tol': 0.05, 'label': '基础3阶段(40iter)'},
        # 更多阶段
        {'stages': [0.5, 0.65, 0.8, 0.9, 1.0], 'max_iter': 20, 'tol': 0.05, 'label': '5阶段(20iter)'},
        {'stages': [0.5, 0.65, 0.8, 0.9, 1.0], 'max_iter': 30, 'tol': 0.05, 'label': '5阶段(30iter)'},
        # 更细粒度
        {'stages': [0.5, 0.6, 0.7, 0.8, 0.9, 1.0], 'max_iter': 20, 'tol': 0.05, 'label': '6阶段(20iter)'},
        {'stages': [0.5, 0.6, 0.7, 0.8, 0.9, 1.0], 'max_iter': 30, 'tol': 0.05, 'label': '6阶段(30iter)'},
        # 更保守的 tol
        {'stages': [0.5, 0.75, 1.0], 'max_iter': 30, 'tol': 0.03, 'label': '3阶段_tol0.03'},
        {'stages': [0.5, 0.65, 0.8, 0.9, 1.0], 'max_iter': 30, 'tol': 0.03, 'label': '5阶段_tol0.03'},
        # 2阶段
        {'stages': [0.5, 1.0], 'max_iter': 40, 'tol': 0.05, 'label': '2阶段(40iter)'},
        {'stages': [0.5, 1.0], 'max_iter': 50, 'tol': 0.05, 'label': '2阶段(50iter)'},
    ]
    ...
    for i, exp in enumerate(experiments):
        ...
        result = run_experiment(
            acc, n, dt, zeta, P_ctrl, nP_ctrl, SPAT_ctrl, periods, target_sa, PGA,
            stages=exp['stages'], max_iter=exp['max_iter'], tol=exp['tol'], label=exp['label']
        )
        all_results.append(result)
        status = "✅" if result['aerror'] < 0.20 else "❌"
        print(f"      PGA={result['PGA_final']:.4f}g, aerror={result['aerror']:.2%}, merror={result['merror']:.2%}, time={result['time']:.1f}s {status}")
        for sr in result['stage_details']:
            print(f"        阶段 {sr['stage_ratio']}: PGA={sr['PGA']:.4f}g, aerror={sr['aerror']:.2%}")
        print()

    # 汇总
    print("=" * 70)
    print("调优结果汇总（按 mean_error 排序）")
    print("=" * 70)
    sorted_results = sorted(all_results, key=lambda x: x['aerror'])
    for r in sorted_results:
        status = "✅" if r['aerror'] < 0.20 else "❌"
        print(f"{status} {r['label']}: aerror={r['aerror']:.2%}, merror={r['merror']:.2%}, time={r['time']:.1f}s")

    # 保存最佳配置
    best = sorted_results[0]
    print()
    print(f"最佳配置: {best['label']}")
    print(f"  mean_error={best['aerror']:.2%}, max_error={best['merror']:.2%}")
    for sr in best['stage_details']:
        print(f"  阶段 {sr['stage_ratio']}: PGA={sr['PGA']:.4f}g, aerror={sr['aerror']:.2%}")
